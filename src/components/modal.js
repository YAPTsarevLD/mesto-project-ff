...
export function openModal(modal) {
  ...
  }, 1); //добавляем класс открытого попапа с небольшой задержкой для плавности
...
export function addCloseListeners() {
  document.querySelectorAll(".popup").forEach((popup) => {
    ...
    popup.addEventListener("click", (evt) => {
      //слушатель нажатия на оверлей
      if (evt.target === popup) {
        //если пользователь нажал не на содержимое попапа
        closeModal(popup);
      }
    });
  });
  ...
  document.querySelectorAll(".popup__close").forEach((popupCloseButton) => {
    popupCloseButton.addEventListener("click", (evt) => {
      //слушатель нажатия на кнопку закрытия (крестик)
