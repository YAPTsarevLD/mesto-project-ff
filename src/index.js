...
import { enableValidation, clearValidation, validationConfig } from './components/validation.js'; //импортируем функции валидации
import { getUserInfo, getInitialCards, updateProfile, addNewCard, deleteCard, likeCard, unlikeCard, updateAvatar } from './components/api.js'; //импортируем функции-запросы
...
if (profileEditButton && profilePopupForm) {
  profileEditButton.addEventListener('click', () => {
    nameInput.value = profileTitle.textContent;
    jobInput.value = profileDescription.textContent;
    openModal(profilePopupForm);
...
if (addCardButton && newCardPopupForm) {
  addCardButton.addEventListener('click', () => { 
    openModal(newCardPopupForm);
  });
}
